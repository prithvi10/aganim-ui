...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  // Fetch retail calendar from backend
...
export default function DigitalMarketing() {
  ...
  // Visual Marketing mission state
  ...
  // Derive next holiday + campaign code for the RetailCampaignCard
  ...
  const campaignCode = useMemo(() => {
    if (!nextHoliday || !selectedProduct?.id) return '';
    const year = Number(nextHoliday.date.slice(0, 4)) || new Date().getFullYear();
    const category = selectedProduct.productType || 'General';
    return discountCodeName(nextHoliday.name, category, year);
  }, [nextHoliday, selectedProduct?.id, selectedProduct?.productType]);
      ...
      <Layout>
        <Layout.Section>
          <BlockStack gap="400">
                ...
                {selectedProduct && (
                  <div style={{ display: 'flex', justifyContent: 'center' }}>
                    <div className="agent-btn-border-5">
                      ...
                      >
                        {hooksLoading ? 'Generating…' : hooks.length ? 'Regenerate' : 'Generate'}
                      </Button>
                    </div>
                  </div>
                )}
            ...
            {/* Digital Marketing — Social Hooks */}
            <Card>
              <Box padding="400">
                <BlockStack gap="300">
